...
In this paper, I present the `stortingscrape` package for R.
...
I will start this paper by briefly discussing the openly accessible
...
the case caught our eye -- for instance, vote number 61 from the Labor
Party,^[https://stortinget.no/no/Saker-og-publikasjoner/Saker/Sak/Voteringsoversikt/?p=85196&dnid=1]
...
```{r covid_relief, eval=TRUE}
covid_relief <- get_vote("85196")
dim(covid_relief)
```

```{r sleep1, echo=FALSE, eval=TRUE}
...
```{r covid_relief_result, eval=TRUE}
...
In this paper, I have presented the philosophy, scope, usage, and workflow of the 
